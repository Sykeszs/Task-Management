  ...
  useEffect(() => {
    if (user) {
      const unsubscribe = onSnapshot(
        query(collection(db, "tasks"), where("userId", "==", user.uid)), // Filter tasks by userId
        (snapshot) => {
              ...
              <li key={task.id} className="p-4">
                <div>
                  ...
                  <div className="text-sm text-gray-500">
                    <span>
                      Created:{" "}
                      {task.createdAt ? task.createdAt.toDate().toLocaleString() : "N/A"}
                    </span>
                    <br />
                    <span>
                      Completed:{" "}
                      {task.updatedAt ? task.updatedAt.toDate().toLocaleString() : "N/A"}
                    </span>
                  </div>
                </div>
              </li>
